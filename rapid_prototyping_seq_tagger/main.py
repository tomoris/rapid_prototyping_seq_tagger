...
def eval(model, data_container, batch_size, data_type, use_gpu, device):
    ...
    all_predict_list = []
    with torch.no_grad():
        for i, batched_data in enumerate(data_container(data_type, batch_size)):
            if use_gpu:
    ...
    all_scores = get_all_scores(all_predict_list, all_correct_list, data_container.i2l, data_container.i2semil, data_container.semi_markov)
    return all_scores
...
def train(config_file):
    ...
    config_container = importlib.import_module(config_file.replace('.py', '').replace('/', '.')).get_config()
    ...
    optimizer = optim.SGD(model.parameters(), lr=config_container.lr, momentum=config_container.momentum, weight_decay=config_container.weight_decay)
    # optimizer = optim.Adam(model.parameters(), lr=config_container.lr) #, momentum=config_container.momentum, weight_decay=config_container.weight_decay)
    ...
    for epoch in range(config_container.epoch):
        ...
        if data_container.train_file_is_partially_labeled is False:
            all_scores = eval(model, data_container, config_container.batch_size, 'train', config_container.use_gpu, device)
        ...
        if config_container.dev_file is not None:
            all_scores = eval(model, data_container, config_container.batch_size, 'dev', config_container.use_gpu, device)
        ...
        if config_container.test_file is not None:
            all_scores = eval(model, data_container, config_container.batch_size, 'test', config_container.use_gpu, device)
        ...
        for key, item in all_scores.items():
            if type(item) is dict:
                for key_key, item_item in item.items():
                    logger.info('final test corpus eval metric {0} {1} = prec. {2:.4f}, recall {3:.4f}, f {3:.4f}'.format(key, key_key, item_item[0], item_item[1], item_item[2]))
            else:
                logger.info('final test corpus eval metric {0} = {1:.4f}'.format(key, item))
...
def main():
    ...
    parser.add_argument('--mode', help='train or predict', type=str, default='predict')
    parser.add_argument('--config', help='configuration file path', type=str, default=None)
    ...
    if args.mode == 'train':
        ...
        sys.exit('--mode can receive train or predict')
