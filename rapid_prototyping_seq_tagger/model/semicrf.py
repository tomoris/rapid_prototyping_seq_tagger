...
class SemiCRF(nn.Module):
    def __init__(self, word_rep_dim, label_size, max_NE_length, pad_id):
        ...
        if max_NE_length >= 2:
            self.cnn = nn.ModuleList([nn.Conv1d(self.word_rep_dim,
                                                self.word_rep_dim, k + 1) for k in range(self.max_NE_length)])
    ...
    def forward(self, word_rep, mask, label_ids=None, calc_loss=False, each_score_in_PYHSMM=None, utf_char_sents=None):
        ...
        -------
            loss : tensor, size(1), dtype=torch.float
                semi-Markov CRF loss = -log( score(y_gold|x) / sum(y' in Y, score(y'|x) ),
                    if calc_loss is False, then 0.0
                if calc_loss is False, then 0.0
            predict_triplets_list : list,
                len(predict_triplets_list) is batch size, len(predict_triplets_list[b]) is number of triplets
                predicted NE label
                predict_triplets_list[b][t] represents triplet (start word position, end word position, label id)
    ...
    def _semicrf_forward(self, label_score, mask, each_score_in_PYHSMM=None, utf_char_sents=None):
        ...
        -------
            log_alpha : tensor,
                size(batch size, maximum length of word tokens +1, self.max_NE_length, label size),
                dtype=torch.float
                semi-Markov crf forward score,
                    log_alpha[:, t, k, l] represents l-th label score of token sequence from (t-k)-th to t-th token
        ...
        if each_score_in_PYHSMM is not None:
            # [l (z)][ll (r)] is r to z
        ...
        for t in range(seq_len):
            for k in range(self.max_NE_length):
                if (t - k) == 0:  # if previous label is BEGIN
                    ...
                    # make log_alpha_tmp,
                    #    size(batch size, label size, max NE length * label size), dim=2 is summed up in logsumexp later
                    log_alpha_tmp_list = []
                    for kk in range(self.max_NE_length):
                        if each_score_in_PYHSMM is None:
                            log_alpha_tmp = label_score[:, t, k, :].unsqueeze(2).expand(-1, -1, self.label_size) \
                                + self.T.expand(batch_size, -1, -1) \
                                + log_alpha[:, t - (k + 1), kk, :].unsqueeze(1).expand(-1, self.label_size, -1)
                        else:
                            phrase_scores_in_PYHSMM = []
                            for b in range(batch_size):
                                phrase_scores_in_PYHSMM.append([float('-inf') for _ in range(self.label_size)])
                                for l in range(self.label_size):
                                    if t < len(utf_char_sents[b]):
                            ...
                            phrase_scores_in_PYHSMM = torch.where(phrase_scores_in_PYHSMM > min_score_in_PYHSMM,
                                                                  phrase_scores_in_PYHSMM, min_score_in_PYHSMM)
                            phrase_scores_in_PYHSMM = phrase_scores_in_PYHSMM * self.lambda_zero
                            log_alpha_tmp = phrase_scores_in_PYHSMM + \
                                label_score[:, t, k, :].unsqueeze(2).expand(-1, -1, self.label_size) + \
                                self.T.expand(batch_size, -1, -1) + \
                                log_alpha[:, t - (k + 1), kk, :].unsqueeze(1).expand(-1, self.label_size, -1)
                        log_alpha_tmp_list.append(log_alpha_tmp)
                    log_alpha_tmp = torch.cat(log_alpha_tmp_list, dim=2)
    ...
    def _calc_log_gold_path_score(self, label_score, mask, label_ids, each_score_in_PYHSMM=None, utf_char_sents=None):
        ...
        batch_size = label_score.size(0)
        seq_len = label_score.size(1)
        log_gold_alpha = self.log_alpha_item.expand(batch_size, seq_len + 1,
                                                    self.max_NE_length, self.label_size).clone()
        log_zero_score = self.log_zero_score.expand(batch_size, self.label_size).clone()
        ...
        for t in range(seq_len):
            for k in range(self.max_NE_length):
                if (t - k) == 0:  # if previous label is BEGIN
                    ...
                    # replace log(0.0) if l-th label is not gold label in token sequence from
                    # (t-k)-th token to t-th token in b-th batch data
                    multi_hot_label_vector = label_ids[:, t, k, :]
                    # assert(multi_hot_label_vector.sum().item() != 0.0)
                    log_gold_alpha[:, t, k, :] = torch.where(
                        multi_hot_label_vector == 1, log_gold_alpha[:, t, k, :], log_zero_score)
                elif (t - k) < 0:
                    pass
                else:
                    # make log_alpha_tmp size(batch size, label size, max NE length * label
                    # size),  dim=2 is summed up in logsumexp later
                    log_alpha_tmp_list = []
                    for kk in range(self.max_NE_length):
                        if each_score_in_PYHSMM is None:
                            log_alpha_tmp = label_score[:, t, k, :].unsqueeze(2).expand(-1, -1, self.label_size) \
                                + self.T.expand(batch_size, -1, -1) \
                                + log_gold_alpha[:, t - (k + 1), kk, :].unsqueeze(1).expand(-1, self.label_size, -1)
                        else:
                            phrase_scores_in_PYHSMM = []
                            for b in range(batch_size):
                                phrase_scores_in_PYHSMM.append([float('-inf') for _ in range(self.label_size)])
    ...
    def _predict(self, label_score, mask, log_alpha):
        ...
        ----------
            label_score : tensor,
                size(batch size, maximum length of word tokens, maximun NE length, label size), dtype=torch.float
                each label score (log)
            mask : tensor, size(batch size, maximum length of word tokens), dtype=torch.uint8
                mask, 0 means padding
            log_alpha : tensor,
                size(batch size, maximum length of word tokens +1, self.max_NE_length, label size), dtype=torch.float
                semi-Markov crf forward score,
                log_alpha[:, t, k, l] represents l-th label score of token sequence from (t-k)-th to t-th token
                last value in dim=1 is padding need to calc. all path score
        returns
        -------
            predict_triplets_list : list, len(predict_triplets_list) is batch size,
                len(predict_triplets_list[b]) is number of triplets
                predicted NE label
                predict_triplets_list[b][t] represents triplet (start word position, end word position, label id)
                    of t-th triplet in b-th batch data
                predict_triplets_list[b][0][0] is 0, predict_triplets_list[b][-1][0] is length of the sentence
        ...
        predict_list = []
        current_token_idx = (torch.sum(mask[:, :, 0].long(), dim=1)).unsqueeze(
            1).unsqueeze(1).expand(batch_size, seq_len, self.label_size)
        ...
        while ((current_token_idx[:, 0, 0] == 0).sum().item() != batch_size):
            ...
            for k in range(self.max_NE_length):
                log_alpha_tmp = torch.gather(log_alpha[:, :, k, :], 1, torch.where(
                    current_token_idx - (k + 1) >= 0, current_token_idx - (k + 1), padding_position_ids))[:, 0, :]
            ...
            prev_token_idx = current_token_idx - \
                (max_k.unsqueeze(1).unsqueeze(1).expand(batch_size, seq_len, self.label_size) + 1)
            predict_list.append(torch.cat([prev_token_idx[:, 0, 0].unsqueeze(
                1), current_token_idx[:, 0, 0].unsqueeze(1) - 1, max_label_id.unsqueeze(1)], dim=1))
    ...
    def _log_sum_exp(self, alpha):
        ...
        log_alpha_item = max_score + \
            torch.log(torch.sum(torch.exp(alpha - max_score.unsqueeze(2).expand_as(alpha)), dim=2))
