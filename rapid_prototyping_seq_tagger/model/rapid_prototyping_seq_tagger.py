...
class Rapid_prototyping_seq_tagger(nn.Module):
    def __init__(self, config_container, data_container):
        ...
        self.semicrf = SemiCRF(self.word_rep_dim, data_container.get_label_vocab_size(), config_container.max_NE_length, data_container.pad_id)
    ...
    def forward(self, token_ids, char_ids, mask, multi_hot_label_tensor=None, calc_loss=False):
        ...
        ----------
            ...
            char_ids : tensor, size(batch size, maximum length of word tokens, maximum length of char tokens), dtype=torch.long
            ...
            multi_hot_label_tensor : tensor, size(batch size, maximum length of word tokens, max NE length, label_size), dtype=torch.uint8
